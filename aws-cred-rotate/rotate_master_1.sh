...
function process_key() {
	...
	if [ $kdate -le $exp_date ]; then
		...
		# set old key inactive - No NEED to set it Inactive
		...
		export CI_ROTATE_NEW_SECRET_KEY="$new_secret_key";
                export CI_ROTATE_ACTION="";
		echo "Update jenkins credential with the newly created AccessKey.";
		groovy "$script_path/credentials_master_1.groovy";
...
function check_key() {
   ...
   if [ -z "${kaccesskey}" ]; then
	(>&2 echo "FATAL: Missing required values to proceed.");
	exit 1;
   fi

   export CI_ROTATE_ACTION="checkkey";
   export CI_ROTATE_CREDENTIAL_ID=$credential_id;
   export CI_ROTATE_OLD_ACCESS_KEY_ID=$kaccesskey;
...
function run_rotate() {
	...
	for key in $current_keys; do
		...
		IFS=, read xusername xcreatedate xaccesskey <<< $key;
                output=$(check_key "$xaccesskey" "$script_path")
                if [ -z "${output}" ]; then
         		# delete old key since this was successful.
	        	aws iam delete-access-key --access-key-id "$xaccesskey" --user-name "$xusername";
                else
                        key_to_update="$key";    
                fi
	done
	IFS=, read xusername xcreatedate xaccesskey <<< $key_to_update;
